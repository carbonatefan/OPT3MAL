...
%%%% The expected input file formats are the following columns in order: 
    %%%% CalibrationDataset: Sample_ID C10 i-C11 a-C11 C11 i-C12
    %%%% C12 i-C13 a-C13 C13 i-C14 C14 i-C15 a-C15 C15 i-C16 C16 i-C17
    %%%% a-C17 C17 i-C18 C-18 MAAT pH
    %%%% 
    %%%% UserDataset: Sample_ID C10 i-C11 a-C11 C11 i-C12 C12 i-C13 a-C13
    %%%% C13 i-C14 C14 i-C15 a-C15 C15 i-C16 C16 i-C17 a-C17 C17 i-C18 C-18
    %%%% 
...
%Choose input files
%User data:
UserDataset='OPT3MAL_Demo.csv';
%Modern calibration dataset:
CalibrationDataset='3_OH_FA_CalibrationData.csv';

%Set filenames for outputs
OPT3MAL_Results='OPT3MAL_Demo_Output.csv'; % csv file containing original sample_ID and 3-OH-FA data plus: Nearest Neighbour distance (MAAT), MAAT prediction, 1 StDev (MAAT), Nearest Neighbour distance (pH), pH prediction, 1 StDev (pH)
...
UserData=csvread(UserDataset,1,1);

%Read in sample IDs
fid = fopen(UserDataset);
formatspec = ['%s',repmat('%*s',1,22)];
Sample = textscan(fid,formatspec,'Delimiter',',');
fid = fclose(fid);
Sample_ID = Sample{:};
Sample_ID = Sample_ID(2:end);

%Read in calibration dataset
Calibration=csvread(CalibrationDataset,1,1);

%GPR based on all compounds or just C15 and C17?
if AllCompounds==true
    input=Calibration(:,1:21); 
    ancient=UserData(:,1:21);
elseif AllCompounds==false
    input=Calibration(:,[12,13,14,17,18,19]); %C15 and C17 compounds only
...
fulloutput={Sample_ID, output};
col_header={'Sample_ID','C10','i-C11','a-C11','C11','i-C12','C12','i-C13','a-C13','C13','i-C14','C14','i-C15','a-C15','C15','i-C16','C16','i-C17','a-C17','C17','i-C18','C-18','D_nearest_MAAT','MAAT','StDevMAAT','D_nearest_pH','pH','StDevpH'};
fileID=fopen(OPT3MAL_Results, 'w');
fprintf(fileID, '%s, ', col_header {:});
fprintf(fileID, '\n');

for k1 = 1:size(Sample_ID,1);
    fprintf(fileID, '%s, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f, %f \n', char(fulloutput{1}(k1)),fulloutput{2}(k1,:));
end
fclose(fileID);